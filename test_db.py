...
zz = [9]*768
def addNewUserInformation(userID, _embedding=zz, _date="2024-01-01 00:00:00", _date_before="2025-12-31 11:59:59"):
...
def updateUserInformation(userID, _embedding=zz, _date="2024-01-01 00:00:00", _date_before="2025-12-31 11:59:59"):
    userSetting = GetUserMethod("user-collections", userID)
    print("update user info : ", _embedding)
    if _embedding == zz:
        _embedding = zz #userSetting["embedding"]
...
def addNewGroup(GroupID):
    Putmethod(GroupID, "Group")
    if PostMessageMethod(GroupID, "1", zz, 1, "2024-01-01 00:00:00"):
        # await asyncio.sleep(1)
        return True
    else:
        return False

def addMessage(GroupID, _message, _embedding, _date, _user="Admin", _url=""):
    GroupMcount = int(GetMessageMethod(GroupID, zz, 1)['message'])

    # print("test.py embedding: ", _embedding)
    if not PostMessageMethod(GroupID, _message, _embedding, GroupMcount+1, _date, _user, _url):
        return False
    if not PostMessageMethod(GroupID, str(GroupMcount+1), zz, 1, "2024-01-01 00:00:00"):
        return False
...
def queryMessage(userID, GroupID, _embedding=zz, _date="2024-01-01 00:00:00", _user="Admin", _date_before="2025-12-31 11:59:59"):
    userSetting = GetUserMethod("user-collections", userID)
...
def GetUserSetting(userID):
    return GetUserMethod("user-collections", userID)

def GetGruopInfo(GruopID):
    return GetMessageMethod(GruopID, zz, 1) 
